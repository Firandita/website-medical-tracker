<template>
  <aside
    class="sidebar w-64 flex-shrink-0 flex flex-col h-full overflow-y-auto z-20"
  >
    ...
    <div class="flex items-center gap-3 px-5 py-5 border-b border-white/10">
      ...
      >
        <div
          class="w-9 h-9 gradient-teal rounded-xl flex items-center justify-center shadow-lg flex-shrink-0"
        >
          <svg
        ...
        <div>
          <p class="font-display font-bold text-white text-sm leading-tight">
            MedTrack 3T
          </p>
      ...
      <button
        class="lg:hidden p-1.5 text-slate-400 hover:text-white hover:bg-white/10 rounded-lg transition"
        @click="$emit('closeSidebar')"
      >
        <svg
          class="w-4 h-4"
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5">
      <p
        class="text-slate-500 text-xs font-semibold uppercase tracking-widest px-3 mb-2 mt-1"
      >
        Menu Utama
      </p>
      ...
      >
        ...
        <span
          v-if="item.badge"
          class="ml-auto bg-brand-500 text-white text-xs rounded-full px-2 py-0.5 font-semibold"
        >
          {{ item.badge }}
        </span>
      </RouterLink>
      ...
      <p
        class="text-slate-500 text-xs font-semibold uppercase tracking-widest px-3 mb-2 mt-4"
      >
        Kecerdasan Buatan
      </p>
      ...
      <button
        class="sidebar-item w-full relative"
        @click="
          emit('openNotif');
          emit('closeSidebar');
        "
      >
        ...
        <span
          v-if="unreadCount >= 0"
          class="ml-auto bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold badge-pulse"
        >
          {{ unreadCount }}
        </span>
      </button>
    ...
    <!-- User profile -->
    <div class="px-3 py-4 border-t border-white/10" @click="goToProfile">
      <div
        class="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 cursor-pointer transition"
      >
        <div
          class="w-8 h-8 gradient-teal rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0"
        >
          A
        </div>
...
// Emits & Props
const emit = defineEmits(["openNotif", "closeSidebar"]);
