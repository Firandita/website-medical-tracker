  ...
  <div class="space-y-5 animate-[slideIn_0.3s_ease]">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <StatCard
        ...
        :progress="
          wilayahStats
            ? Math.round((wilayahStats.kritis / wilayahStats.total) * 100)
            : 0
        "
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
      <!-- Map card -->
      <div
        class="xl:col-span-2 card-glass rounded-2xl shadow-card overflow-hidden"
      >
        <div
        ...
        >
          <div>
            <h3 class="font-display font-semibold text-slate-800">
              Peta Sebaran Dokter per Kabupaten 3T
            </h3>
            <p class="text-slate-400 text-xs mt-0.5">
              Klik marker untuk melihat detail kabupaten/kota
            </p>
          </div>
          <div class="flex items-center gap-2">
            <button
              :class="[
                'text-xs px-3 py-1.5 rounded-lg shadow-sm transition',
                mapMode === 'cluster'
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
      ...
      <div class="xl:col-span-2 card-glass rounded-2xl shadow-card p-5">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="font-display font-semibold text-slate-800">
              Tren Sebaran Dokter (6 Bulan)
            </h3>
            <p class="text-slate-400 text-xs">
              Perkembangan jumlah dokter di kabupaten/kota wilayah 3T
            </p>
          </div>
        </div>
      ...
      <div class="card-glass rounded-2xl shadow-card overflow-hidden">
        ...
        >
          <div>
            <h3 class="font-display font-semibold text-slate-800 text-sm">
              Peringatan Aktif
            </h3>
            <p class="text-slate-400 text-xs">Kabupaten/kota perlu perhatian</p>
          </div>
          <span
            class="bg-red-50 text-red-500 text-xs font-semibold px-2 py-0.5 rounded-full"
            >{{ wilayahKritis.length }} kritis</span
          >
        </div>
        <div
          class="divide-y divide-slate-50 overflow-y-auto"
            ...
            <div class="flex items-start gap-2">
              ...
              <div>
                ...
                <p class="text-slate-500 text-xs">
                  {{ w.jumlahDokter }} dokter untuk
                  {{ w.populasi.toLocaleString("id-ID") }} jiwa
                </p>
                <div class="flex gap-1 mt-1.5 flex-wrap">
                  <span class="badge-critical">KRITIS</span>
                  <span
                    class="text-slate-500 bg-slate-100 text-xs px-2 py-0.5 rounded-full"
                    >{{ w.aksesibilitas }}</span
                  >
                </div>
              </div>
            </div>
        ...
        <div class="px-4 py-3 border-t border-slate-100">
          <button
            class="text-brand-600 text-xs font-semibold hover:text-brand-700 transition"
            @click="$router.push('/wilayah')"
          >
            Lihat semua wilayah →
          </button>
    ...
    <div class="card-glass rounded-2xl shadow-card overflow-hidden">
      ...
      >
        <div>
          <h3 class="font-display font-semibold text-slate-800">
            Dokter Terbaru Ditugaskan
          </h3>
          <p class="text-slate-400 text-xs">
            Penugasan dokter di kabupaten/kota wilayah 3T
          </p>
        </div>
        <button
          class="btn-primary text-xs py-1.5"
          @click="$router.push('/dokter')"
        >
          Lihat Semua →
        </button>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody>
            <tr
              v-for="d in dokterList.slice(0, 5)"
              :key="d.id"
              class="table-row"
            >
              <td class="table-cell">
...
<script setup>
import { onMounted, ref, computed } from "vue"; // Tambahkan computed
...
// mapData diambil dari wilayahStore via Pinia storeToRefs
...
onMounted(async () => {
  // Menunggu inisialisasi kedua store selesai
  await Promise.all([wilayahStore.init(), dokterStore.init()]);
  wilayahStats.value = wilayahStore.stats;
  stats.value = dokterStore.stats;
  buildChart();
});

// Pastikan data yang dikirim ke LeafletMap selalu terbaru
...
function buildChart() {
  ...
  new Chart(trendChartRef.value, {
    ...
    options: {
      ...
      plugins: {
        legend: {
          labels: { font: { family: "DM Sans", size: 11 }, color: "#64748b" },
        },
      },
      scales: {
        x: {
